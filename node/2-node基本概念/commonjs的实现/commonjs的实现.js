...
/* 
    模块的实现：1. 实现一个 require 方法
               ...
               5. 会尝试加载是不是一个原生模块，如果带相对路径、绝对路径，那么就不是核心模块 
               6. 根据文件路径创建一个模块（一个模块必须要有三个属性 id,path,exports【后续文件导出的结果需要保存到这个变量上】）
               7. 根据创建的模块，进行模块的加载
               8. 加载模块时会构建一个 paths 属性，这个属性就是第三方模块的查找路径
               9. 取出文件的后缀 Module._extensions 调用对应的加载模块的策略
               10.读取文件内容
               11.在文件内容外面包裹一个函数
               12.exports === Module.exports【最终require方法会拿到 module.exports 返回结果】
            重要：最终用户使用的结果都来自于 module.exports 如果只是改变exports的值，不会影响 module.exports 的值，但是
                  给 exports 增添属性【exports.a = 1】，是可以修改 module.exports。注意不要 exports 和 module.exports 混淆使用
                  结果都是以 modlue.exports 为准 
*/
...
// 	console.log(r); 
// },1000) 
// es6模块 使用export {} 导出的是一个变量（接口），如果内部对应的值发生变化是由影响的
